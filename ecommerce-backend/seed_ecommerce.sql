...
('user_003_buyer', 'buyer@example.com', true, '$2b$10$WpIyBFDZB5l1eiZUKae7N.a7DPg0Jr8ZbRHJvfNdI8iLWwzx7jnYa', 'USER', 'ACTIVE', now(), now()) ON CONFLICT (id) DO NOTHING;
...
('prof_003_buyer', 'user_003_buyer', 'buyer@example.com', 'Buyer User', 'USER', 'ACTIVE', now(), now()) ON CONFLICT (id) DO NOTHING;

-- Insert into Seller Profiles
INSERT INTO seller_profiles (seller_id, shop_name, account_type, status, is_verified, created_at) VALUES
('user_002_seller', 'Super Shop', 'BUSINESS', 'ACTIVE', true, now()) ON CONFLICT (seller_id) DO NOTHING;
