...
interface Props {
  ...
  profileCache: Record<string, { id: string; fullName?: string; avatarUrl?: string }>;
  isSeller: boolean;
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const groupTargets: Target[] = groups.map((g) => ({
          id: g.groupId,
          type: 'group' as const,
          label: g.name,
        ...
        const convTargets: Target[] = conversations
          ...
          .map((c) => {
            ...
            const label = profile?.fullName || (isSeller ? 'Người mua' : 'Người bán');
            return {
              id: c.id,
              type: 'conv' as const,
              label,
        ...
        setTargets(convTargets);
      } finally {
        setLoading(false);
      }
    };
    load();
  }, [conversations, currentConversationId, isSeller, profileCache, user?.id]);
  ...
  const handleSendItem = async (t: Target) => {
    ...
    try {
      ...
      const senderName = message.senderId === user?.id
        ? 'Bạn'
        : (profileCache[message.senderId]?.fullName || (isSeller ? 'Người bán' : 'Người mua'));
