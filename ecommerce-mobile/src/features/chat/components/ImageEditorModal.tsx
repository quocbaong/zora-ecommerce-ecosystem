...
import { 
  X, RotateCw, RefreshCw, Pencil, Smile, 
...
export default function ImageEditorModal({ visible, imageUri, onCancel, onDone }: Props) {
  ...
  const handleNativeCrop = async (aspectRatio?: number) => {
    try {
      const options: any = {
        path: currentUri,
        mediaType: 'photo',
      ...
      if (aspectRatio) {
        options.width = 1000;
        options.height = Math.round(1000 / aspectRatio);
        options.freeStyleCropEnabled = false;
      } else {
        options.freeStyleCropEnabled = true;
      }

      const image = await ImageCropPicker.openCropper(options);
      if (image && image.path) {
        setCurrentUri(image.path);
      }
    } catch (e) {
                  ...
                  <View className="flex-row gap-4 items-center">
                    ...
                    {ASPECT_OPTIONS.map((opt, i) => (
                      <TouchableOpacity 
                        key={i} 
                        className="p-2"
                        onPress={() => handleNativeCrop(opt.value)}
                      >
                        <Text className="text-white/60 text-[10px] font-bold uppercase">{opt.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                )}
