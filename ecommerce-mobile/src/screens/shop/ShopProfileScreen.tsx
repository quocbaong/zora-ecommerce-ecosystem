...
const ShopProfileScreen = ({ route, navigation }: any) => {
  ...
  const renderHeader = () => (
    <View>
      ...
      <View className="px-4 pb-4 bg-white rounded-b-3xl shadow-sm mb-4">
        <View className="flex-row justify-between items-start -mt-8 mb-4">
          <View className="w-20 h-20 rounded-full bg-white border-4 border-white overflow-hidden shadow-sm">
            <Image 
              source={{ uri: `https://ui-avatars.com/api/?name=${encodeURIComponent(sellerId)}&background=random&size=150` }} 
              className="w-full h-full" 
            />
          </View>
          <TouchableOpacity className="mt-10 px-6 py-2 bg-primary rounded-full">
            <Text className="text-white font-bold text-xs">+ Theo dõi</Text>
          </TouchableOpacity>
        </View>

        <Text className="text-xl font-bold text-secondary mb-1">Zora Official Store</Text>
        <View className="flex-row items-center mb-4">
          <MapPin size={12} color={COLORS.textSecondary} />
          <Text className="text-gray-500 text-xs ml-1 mr-3">TP. Hồ Chí Minh</Text>
          <Star size={12} color="#FBBF24" fill="#FBBF24" />
          <Text className="text-gray-500 text-xs ml-1">4.9/5.0</Text>
        </View>

        {/* Stats Grid */}
        <View className="flex-row justify-between border-t border-gray-100 pt-4">
          <View className="items-center flex-1">
            <Text className="text-secondary font-bold text-base">24K</Text>
            <Text className="text-gray-400 text-[10px] mt-1">Người theo dõi</Text>
          </View>
          <View className="w-[1px] h-full bg-gray-100" />
          <View className="items-center flex-1">
            <Text className="text-secondary font-bold text-base">{products.length}+</Text>
            <Text className="text-gray-400 text-[10px] mt-1">Sản phẩm</Text>
          </View>
          <View className="w-[1px] h-full bg-gray-100" />
          <View className="items-center flex-1">
            <Text className="text-secondary font-bold text-base">2023</Text>
      ...
      {/* Internal Search */}
      <View className="px-4 py-3 bg-white mb-2 flex-row items-center justify-between shadow-sm">
        <Text className="text-secondary font-bold text-base">Sản phẩm</Text>
