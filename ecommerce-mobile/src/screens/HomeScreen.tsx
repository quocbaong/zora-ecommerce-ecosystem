...
const HomeScreen = ({ navigation }: any) => {
  ...
  const loadData = async () => {
    try {
      const prodRes = await productApi.getProducts({ size: 10 });
      ...
      >
        ...
        <View className="px-4 py-2">
          <View className="bg-white flex-row items-center px-4 py-3 rounded-2xl shadow-sm border border-gray-100">
            ...
            <TextInput 
              placeholder="Tìm kiếm sản phẩm trên ZORA..." 
              className="ml-3 flex-1 text-sm text-secondary font-medium"
              ...
              {CATEGORIES.map((c) => (
                ...
                >
                  <View className="w-14 h-14 rounded-full overflow-hidden mb-2 bg-gray-50">
                    <Image source={c.image} className="w-full h-full" resizeMode="cover" />
                  </View>
        ...
        <View className="mt-8 px-4 pb-32">
          <View className="flex-row items-center justify-between mb-6 border-b-2 border-primary pb-3">
            <View className="flex-row items-center">
